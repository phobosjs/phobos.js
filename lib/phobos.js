...
class Phobos {
  constructor(options) {
    ...
    if (!this._options.testMode) {
      console.info("Phobos, mongoose version:", mongoose.version);
    }
  ...
  /**
   * Cleanup method intended for use in test environments.
   * - Disconnects from MongoDB
   * - Removes process signal handlers
   * - Clears internal references
   *
   * Should only be used in test teardown logic. Not intended for production use.
   */
  async destroy() {
    // Disconnect from MongoDB if connected
    if (this.database.connection.readyState !== 0) {
      await this.database.disconnect();
    }

    // Remove SIGINT/SIGTERM listeners if they were added
    if (this._gracefulExit) {
      process.off("SIGINT", this._gracefulExit);
      process.off("SIGTERM", this._gracefulExit);
      this._gracefulExit = null;
    }
