...
class Phobos {
  ...
  constructor(options) {
    ...
    this.server = restify.createServer({
      name: this._options.appName
    });

    this.serverPlugins = require('../plugins/server');
    ...
    this.token = new TokenLib(this._options.bearerTokenSignature);
  ...
  initDb() {
    ...
    let Schemas = (typeof this.schema === 'function') ? this.schema(this.database) : {};
  ...
  start() {
    ...
    for (let n in this.serverPlugins) {
      this.server.use(this.serverPlugins[n].plugin(this.serverPlugins[n].opts));
    }
  ...
  mountErrorHandler(fn) {
    ...
    if (typeof this._options.errorHandler === 'function') return this.server.use(this._options.errorHandler);
