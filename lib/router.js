...
var Routes = function(express, DS) {
  ...
  Builder.mixInController = function(spec, action, rest, params) {
    return function(req, res, next) {
      var resource = params.model || Inflector.singularize(Inflector.camelize(req.route.path.split('/')[1]));
      var locator = params.locator || { field: '_id' };
  ...
  Builder.route = function(controller) {
    ...
    if (controller._resource) params.model = controller._resource;
    if (controller._locator) params.locator = controller._locator;
  ...
  Builder.runQuery = function(req, res, next) {
    ...
    if (req.params.id) {
      ...
      if (req.controller.locator.field === '_id') {
        q = q.findById(req.params.id);
      } else {
        var search = {};
        search[req.controller.locator.field] = req.params.id;
        q = q.findOne(search);
      }
      ...
      q.lean().exec(function(err, result) {
        ...
        if (req.controller.locator.attachTo && Array.isArray(result[req.controller.locator.attachTo])) {
          var node = result[req.controller.locator.attachTo];

          for (var i = 0; i < node.length; i++) {
            if (node[i]._id.toString() === req.params.id) result = node[i];
          }
        } else if (req.controller.locator.attachTo) {
          result = result[req.controller.locator.attachTo];
        }
  ...
  Builder.filterAllowedFields = function(req, res, next) {
    if (!req.rawResources) return next();
