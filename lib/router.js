...
var Routes = function(express, DS) {
  ...
  Builder.catchScope = function(req, res, next) {
    ...
    if (req.controller._rest) {
      permissions = req.controller.permissions[MethodMatrix[req.method]] || req.controller.permissions;
    }
    ...
    if (req.user) {
      ...
      if (req.caughtScope === '*' && !req.controller._rest && userScopes.length > 1) {
        for (var i = 0; i < userScopes.length; i++) {
          if (req.controller.action.scope.indexOf(userScopes[i]) > -1) req.caughtScope = userScopes[i];
        }
      }
    }
    ...
    if (permissions === true || permissions[req.caughtScope] === true) {
      ...
      req.allowedFields = true;
      return next();
    } else if (!req.controller._rest && req.controller.action.scope.indexOf(req.caughtScope) > -1) {
      req.allowedFields = true;
      return next();
    }
  ...
  Builder.filterAllowedFields = function(req, res, next) {
    if (!req.rawResources || Object.keys(req.rawResources).length === 0) {
      return next();
    }
